...
function setImg() {
	...
	.fadeOut(400, function() {
		$(".screenshot").attr('src', screenshots[appNum][imgInd]);
	    })
	.fadeIn(400);
}

function getAppTitle(num) {
	return appNames[num].replace(/_/g, ' ');
}
...
function loadPortfolio() {
	...
	$("#android_portfolio_text").html($("#"+appNames[appNum]+".hidden_portfolio_text").html());

	function switchImg(incr) {
	    imgInd += incr;
	    if(imgInd < numImgs[appNum] && imgInd >= 0) {
		setImg();
	    } else {
		imgInd -= incr;
	    }
