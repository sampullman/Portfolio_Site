...
function Enemy(E) {
	...
	if(E.attackPathFn) e.attackPath = new E.attackPathFn(e);
	...
	if(E.mode == EnemyMode.ATTACK || E.mode == EnemyMode.INIT ||
	   E.x < 0 || E.x > C_WIDTH || E.attackPath == null) return;
...
function Slider(label, units, x, y, width, height, min, max, step, horizontal, changeListener) {
    this.label = label; this.units = units;
    this.x = x; this.y = y;
    this.width = width; this.height = height;
    this.xKnob = x;
    this.yKnob = horizontal ? y + 20 : y;
    this.step = step;
    this.min = min; this.max = max; this.val = min; this.displayVal = min
...
function Stars(sprite, minSize, maxSize, minSpeed, maxSpeed, freq) {
    this.stars = [];
    this.timer = 0;
    this.sprite = sprite;
	...
	var star = [];
	star.push(x || Math.random()*C_WIDTH);
	star.push(y || 0);
