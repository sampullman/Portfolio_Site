...
import { enemyObjList } from './space_enemies';
import { RandomVerticalInit } from './space_paths';
import { gameState, GameMode } from './space_state';
import { levels } from './space_levels';
...
let playerLivesSlider;
let playerMissilesSlider;
let attackFreqSlider;
let prevX = 0;
let prevY = 0;
let drawFn;
let startGame;
...
function stopCustom(c) {
  ...
  togglePlay.setClickListener(() => { playCustom(c) });
  ...
  gameState.enemies = staticEnemies.slice(0, staticEnemies.length).concat(editorEnemies.slice(0, editorEnemies.length));
...
function staticEnemyClick(enemyObj, enemy) {
  return () => {
    const e = enemyObj.instantiate(enemy.x, enemy.y, enemyObj.sprite.width, enemyObj.sprite.height, 30);
...
function clearSelectedEnemies() {
  selectedEnemies.forEach((e) => { e.squared = false });
  selectedEnemies = [];
}
...
function activateEnemies(c) {
  ...
  gameState.editorActiveEnemies = gameState.editorActiveEnemies.filter(e => e.editorActive);
  editorDraw(c);
}
...
function setupEditor(c, draw, framesPerSecond, startFn, showStart) {
  ...
  FPS = framesPerSecond;
  drawFn = draw;
  keyhandler.fns.del = () => { deleteSelected(c) };
  keyhandler.fns.c = () => { copyEnemy(c) };
  keyhandler.fns.a = () => { activateEnemies(c) };
  staticEnemies = [];
  let x = 0;
  const y = c.boundary + 10;
  for(let i = 0; i < enemyObjList.length; i += 1) {
    const obj = enemyObjList[i];
    const e = obj.instantiate(x, y, obj.sprite.width, obj.sprite.height, 0, RandomVerticalInit);
  ...
  const healthUpdate = (val) => {
    selectedEnemies.forEach((enemy) => { enemy.health = Math.round(val) });
  };
  const shotFreqUpdate = (val) => {
  ...
  const speedUpdate = (val) => {
    selectedEnemies.forEach((enemy) => { enemy.speed = val });
  };
  const sliderWidth = 75;
